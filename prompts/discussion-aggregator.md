...
- **标题清晰**（包含明确的技术术语、产品完整描述、主题说明）：
  - 使用**标题 + 帖子描述（如果 API 提供）**进行筛选

- **标题模糊**（单个词、缩写、不明确的短语）：
  - 访问原文链接，提取页面关键信息：页面标题、meta description、主要可见文本（前几段或前 500 字）
  - 如果原文无法访问或提取失败，则获取讨论区前 3-5 条评论
  - 使用**标题 + 原文页面信息或评论**进行筛选
